...
public class XMLOutputFormatter extends AbstractFormatter {
	...
	public Document formatPDFText(byte[] pdfContent) throws DocumentException,
		...
		if (this.isKeepPage()) {
			...
			List<Element> pageElements = sourceRoot
					.selectNodes(PdfFormatConstants.PAGE_XPATH_EXP);
			for (Element page : pageElements) {
				Element recordsElement = formatedRoot
						.addElement(PdfFormatConstants.RECORDS_ELEMENT);
				recordsElement.addAttribute(PdfFormatConstants.PAGE_NO_ATTR,
						page.attributeValue(PdfFormatConstants.PAGE_ATTR));
				treeWalkBlockElement(page, recordsElement,
						PdfFormatConstants.BLOCK_XPAHT_BY_PAGE_EXP);
			}
		} else {
			Element recordsElement = formatedRoot
					.addElement(PdfFormatConstants.RECORDS_ELEMENT);
			treeWalkBlockElement(sourceRoot, recordsElement,
					PdfFormatConstants.BLOCK_XPATH_EXP);
		}
		return formatedDoc;
	}

	private void treeWalkBlockElement(Element sourceElement,
			Element recordsElement, String xpath) {
		@SuppressWarnings("unchecked")
	...
	private void splitTextToCol(Element recordsElement, String text) {
		...
		for (String line : wordLines) {
			if (!StringUtils.isEmpty(line.trim())) {
				Element recordElement = recordsElement
						.addElement(PdfFormatConstants.RECORD_ELEMENT);
				String[] wordsPerLine = line.split(this
						.generateSpaceSplitRegex());
				for (String word : wordsPerLine) {
					if (!StringUtils.isEmpty(word)) {
						Element colElement = recordElement
								.addElement(PdfFormatConstants.COLUMN_ELEMENT);
	...
	private void handleTableBlock(Element recordsElement, Element element) {
		Element tablElement = recordsElement
				.addElement(PdfFormatConstants.TABLE_ELEMENT);
		@SuppressWarnings("unchecked")
		...
		while (rowYposIterator.hasNext()) {
			Element recordElement = tablElement
					.addElement(PdfFormatConstants.RECORD_ELEMENT);
			@SuppressWarnings("unchecked")
			List<Element> rowBlockElements = element
					.selectNodes("./block[@ypos=" + rowYposIterator.next()
							+ "]");
	...
	private LinkedHashSet getRowYposForTable(Element tableBlock) {
		...
		for (Element block : blockList) {
			String value = block
					.valueOf(PdfFormatConstants.BLOCK_YPOS_XPATH_EXP);
	...
	private List getTableBlock(Element tableBlock) {
		@SuppressWarnings("unchecked")
		List<Element> blockList = tableBlock
				.selectNodes(PdfFormatConstants.CELL_BLOCK_XPATH_EXP);
		return blockList;
	}
}
